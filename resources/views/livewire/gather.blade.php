@if($allowed)
    <div id="level{{ $resourceId }}" class="text-center p-4 overflow-hidden shadow-xl {{($canEnable) ? 'bg-blue-200' : 'bg-gray-900'}} rounded-b-lg ">
        <div class="title">
            <h2 class="p-1 font-semibold text-xl text-left {{($canEnable) ? 'text-black' : 'text-white'}} leading-tight">{{ $resourceName }}</h2>
